...
function SoundsScreen({ currentSound, setCurrentSound }) {
  ...
  const loadedSoundIdRef = useRef(null);
  ...
  // Останавливаем старый звук, если выбранный трек поменялся "снаружи" (например, через
  // "Быстрый запуск" на главной), а не через клик по карточке внутри этого экрана
  useEffect(() => {
